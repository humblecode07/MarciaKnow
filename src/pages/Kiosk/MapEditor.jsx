...
const MapEditor = () => {
  ...
  const renderSavedPaths = (g) => {
    ...
    savedPaths.forEach(path => {
      ...
      g.append("rect")
        .attr("x", path.points[0].x - 10) // Adjust for positioning
        .attr("y", path.points[0].y - 10)
        .attr("width", 40)  // Set the width
        .attr("height", 25) // Set the height
        .attr("fill", "#1a237e") // Optional: fill color

      // Add start point marker (always visible)
      g.append("path")
        .attr("d", "M14 21V19H19V16H10V3H30V16H21V19H26V21H14ZM12 14H28V5H12V14")
        .attr("transform", `translate(${path.points[0].x - 10}, ${path.points[0].y - 10})`)
        .attr("fill", "#FFFFFF")
